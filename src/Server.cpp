	...
	GGKServerDataGetter getter, GGKServerDataSetter setter)
{
	...
	std::string objectPath {getServiceName()};
	std::replace(objectPath.begin(), objectPath.end(), '.', '/');
	objects.push_back(DBusObject(DBusObjectPath() + "com" + objectPath));
	...
	.gattServiceBegin("GattServer1", "FDEAFDEA-1E3C-FAD4-74E2-97A033F1BFAA")

		// Characteristic: String value (custom: 00000002-1E3C-FAD4-74E2-97A033F1BFAA)
		.gattCharacteristicBegin("write", "FDEA0001-1E3C-FAD4-74E2-97A033F1BFAA", {"write", "write-without-response"})

			// Standard characteristic "ReadValue" method call
			.onReadValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				const char *pTextString = self.getDataPointer<const char *>("GattServer1/write", "");
			...
			.onWriteValue(CHARACTERISTIC_METHOD_CALLBACK_LAMBDA
			{
				...
				self.setDataPointer("GattServer1/write", Utils::stringFromGVariantByteArray(pAyBuffer).c_str());
		...
		.gattCharacteristicBegin("state", "FDEA0002-1E3D-FAD4-74E2-97A033F1BFEE", {"notify"})

			.onUpdatedValue(CHARACTERISTIC_UPDATED_VALUE_CALLBACK_LAMBDA
			{
				self.sendChangeNotificationValue(pConnection, std::to_string(buzzerState));
				return true;
			})
...
void Server::SetBuzzerState(int state)
{
	buzzerState = state;
}
